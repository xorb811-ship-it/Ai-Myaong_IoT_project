  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={skip}
        disabled={finishing}
        className="absolute top-5 right-5 z-[103] inline-flex items-center gap-1 rounded-full bg-brand-card/90 px-3 py-1.5 text-xs font-bold text-brand-mute shadow-soft touch-active disabled:opacity-60"
      >
      ...
      {/* 말풍선 (꼬리 달린 툴팁) */}
      <div
        className="absolute z-[101] rounded-3xl bg-brand-card p-5 shadow-soft-lg transition-all duration-300 ease-out"
        style={{ left, width: TOOLTIP_W, ...vStyle }}
      >
        {r && (
          <span
            className="absolute w-3.5 h-3.5 bg-brand-card rotate-45"
            style={{ left: beakLeft - 7, ...(placeBelow ? { top: -6 } : { bottom: -6 }) }}
          />
        )}
        ...
        <h3 className="relative mt-3 font-display text-lg font-bold text-brand-brown">{step.title}</h3>
        <p className="relative mt-1.5 text-sm text-brand-mute leading-relaxed">{step.content}</p>
        ...
        <div className="relative mt-5 flex gap-2.5">
          <button
            ...
            className="inline-flex items-center justify-center gap-1 rounded-2xl bg-brand-cream px-4 py-2.5 text-sm font-bold text-brand-brown touch-active disabled:opacity-40"
          >
